...
function showMagazineGUI(state, plr)
...
function takeMagazineOne(item)
	...
	local _, count = takeMagazineItem(bid, item, true)
	item.count = count
	if count then
		exports["imta-eq"]:giveItem(client, item)
		triggerEvent("onLogsSomething", root, "outputLogs", "magazines", "WYCIĄGNIĘCIE Z MAGAZYNU WYCIĄGAJĄCY: "..getElementData(plr, "character:id").." BID: "..bid.." RZECZ: "..toJSON(item))
		triggerClientEvent(client, "createClientNotification", client, "Przedmiot zabrany!", "Właśnie zabrałeś jeden z przedmiotów z magazynku." , "tick", nil, nil, 85, 204, 0)
	else
		triggerClientEvent(plr, "createClientNotification", plr, "Operacja nieudana", "Przedmiot który próbujesz wyciągnąć z magazynu, już się w nim nie znajduje!", "error")
	end
	triggerClientEvent(plr, "onServerDemandShowMagazine", resourceRoot, true, getMagazineData(bid))
end
addEvent("onPlayerDemandMagazineTakeOne", true)
addEventHandler("onPlayerDemandMagazineTakeOne", resourceRoot, takeMagazineOne)

function takeMagazineAll()
	local plr = client
	...
	local items = takeAllMagazineItems(bid)
	for i, item in ipairs(items) do
		exports["imta-eq"]:giveItem(client, item)
	end
	triggerEvent("onLogsSomething", root, "outputLogs", "magazines", "WYCIĄGNIĘCIE Z MAGAZYNU WSZYSTKO WYCIĄGAJĄCY: "..getElementData(plr, "character:id").." BID: "..bid.." RZECZY: "..toJSON(items))
	triggerClientEvent(client, "createClientNotification", client, "Magazynek opróżniony!", "Zabrałeś wszystko co się dało z magazynu. Miejmy nadzieję, że właściciel budynku o tym wie :)" , "tick", nil, nil, 85, 204, 0)

	triggerClientEvent(plr, "onServerDemandShowMagazine", resourceRoot, true, getMagazineData(bid))
end
addEvent("onPlayerDemandMagazineTakeAll", true)
addEventHandler("onPlayerDemandMagazineTakeAll", resourceRoot, takeMagazineAll)
