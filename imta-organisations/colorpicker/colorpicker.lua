...
function colorPicker.create(id, start, title, editOrganisation)
  local cp = {}
  setmetatable(cp, colorPicker)
  cp.id = id
  cp.editOrganisation = editOrganisation
  cp.color = {}
  ...
  cp.gui.okb = guiCreateButton(336, 235, 64, 24, "Zatwierdź", false, cp.gui.window)
  cp.gui.closeb = guiCreateButton(336, 265, 64, 24, "Cofnij", false, cp.gui.window)
...
function colorPicker:pickColor()
  if not self.editOrganisation then
	onAdminPickColor(self.color.r, self.color.g, self.color.b)
  else 
	onAdminEditPickColor(self.color.r, self.color.g, self.color.b)
  end
  self:destroy()
end
